...
const EDITORIAL_WORD_RANGE = { min: 500, max: 1800 };
const QUALITY_DISCLAIMER = "Editorial checklist for people-first writing. Not a Google ranking score, and it does not guarantee visibility in Search, AI Overviews, or AI Mode.";
...
function expertiseLexicon(locale) {
    const extra = locale ? (EXPERTISE_TERMS_I18N[locale] ?? []) : [];
    return extra.length ? EXPERTISE_TERMS.concat(extra) : EXPERTISE_TERMS;
}
const NEXT_STEP_RE = /\b(contact|enquire|inquiry|rfq|sample|spec(?:ification)?|verify|check|next step|ask for|request)\b|تماس|استعلام|далее|свяж|liên hệ|yêu cầu/i;
const GENERIC_HEADING_RE = /^(introduction|overview|conclusion|summary|final thoughts|in conclusion|مقدمه|نتیجه|введение|заключение|giới thiệu|kết luận)$/i;
function check(id, group, status, label, why, how) {
...
function catalogHas(catalog, href) {
    const [pathname] = href.split(/[?#]/);
    return catalog.some((item) => item.path === pathname);
}
function detectKeywordStuffing(text, keyphrase) {
    ...
    if (collapsed.includes(triple))
        return true;
    if (words > 0 && count >= 5 && count / words > 0.04)
        return true;
    if (count >= 8)
        return true;
    return false;
}
function suggestInternalLinks(input, catalog) {
    const used = new Set();
    for (const href of input.related)
        used.add(href.split(/[?#]/)[0] ?? href);
    ...
    used.add(self);
    const query = significantTokens([input.title, input.focusKeyphrase ?? "", input.excerpt, ...(input.tags ?? []), input.body.slice(0, 1200)].join(" "));
    if (query.length === 0)
        return [];
    const scored = catalog
        ...
        for (const token of query) {
            if (hay.has(token))
                score += 2;
            else if (`${item.title} ${item.path}`.toLowerCase().includes(token))
                score += 1;
        }
        return { item, score };
    })
...
function publishContractChecks(input, catalog) {
    ...
    return [
        check("req_title", "ready", title ? "good" : "required", "Title", "The title is the page H1, the BlogPosting headline, and the default search title.", "Write a specific headline a buyer would recognize. Do not start the body with #."),
        check("req_excerpt", "ready", excerpt ? "good" : "required", "Excerpt / description", "The excerpt is the listing summary and the fallback meta description.", "Write 1–2 sentences that say what the reader will be able to do after reading."),
        check("req_image", "ready", image ? "good" : "required", "Featured image", "A published article needs a real featured image for the page, listings, and default social image.", "Upload a wide photograph to the media library. Do not invent a stock scene that did not happen."),
        check("req_image_alt", "ready", image && imageAlt ? "good" : "required", "Featured image alt text", "Alt text describes the photograph for readers who cannot see it. Keyword stuffing here is harmful.", "Describe what is visibly shown, in the article language."),
        check("req_slug", "ready", slug && SLUG_RE.test(slug) ? "good" : "required", "Slug", "The slug is the stable English URL segment shared across locales.", "Use lowercase letters, numbers, and hyphens only. Keep it identical in every language collection."),
        check("req_date", "ready", date && isValidIsoDate(date) ? "good" : "required", "Publication date", "datePublished and sitemap lastmod (when there is no revision) come from this field.", "Pick a real calendar date. Do not change it later just to look fresh."),
        check("req_updated", "ready", !updated || (isValidIsoDate(updated) && (!date || updated >= date)) ? "good" : "required", "Revision date", "If set, the revision must be on or after publication. It becomes dateModified and sitemap lastmod.", "Set a revision date only when the article actually changed."),
        check("req_canonical", "ready", !canonical || HTTPS_RE.test(canonical) ? "good" : "required", "Canonical URL", "A canonical override must be an absolute https URL. Leave it empty to use this article URL.", "Use this only for a syndicated copy. Hreflang is still generated from real translations."),
        check("req_related", "ready", unknownRelated.length === 0 ? "good" : "required", "Related internal pages", "Unknown paths fail the production build. The site never invents URLs.", unknownRelated.length
        ...
        check("req_seo_title_len", "ready", !seoTitle || seoTitle.length <= 60 ? "good" : "required", "SEO title length", "An override longer than 60 characters is truncated in search results and fails the build.", "Shorten the SEO title, or leave it empty to use the article title."),
        check("req_seo_desc_len", "ready", !seoDescription || seoDescription.length <= 160 ? "good" : "required", "Meta description length", "An override longer than 160 characters fails the build.", "Trim the meta description, or leave it empty to use the excerpt."),
        check("req_no_h1", "ready", headings.some((heading) => heading.level === 1) ? "required" : "good", "Heading hierarchy (no body H1)", "The title field is the only H1. A Markdown # in the body duplicates it.", "Start sections with Heading 2, then Heading 3."),
    ];
}
function searchChecks(input) {
    const title = trimToString(input.title);
    const excerpt = trimToString(input.excerpt);
    const seoTitle = trimToString(input.seoTitle);
    const seoDescription = trimToString(input.seoDescription);
    ...
    const intentCue = /how|what|when|why|vs\.?|versus|check|guide|compare|source|buy|checklist|چگونه|چیست|چه |как |что |làm sao|cách |kiểm tra|so sánh/i.test(title) || significantTokens(title).length >= 4;
    const intentStatus = !title || intentCue ? "good" : "attention";
    return [
        check("search_title", "search", titleStatus, "Title quality and clarity", titleWhy, titleHow),
        check("search_intent", "search", intentStatus, "Search intent", intentStatus === "good"
            ? "The title names a task or question a buyer would actually have."
            : "The title is too generic to match a specific search intent.", "Name the decision or check (for example, what to inspect, or how to compare IQF with block-frozen). Do not write a teaser."),
        check("search_meta", "search", metaStatus, "Meta description quality", metaWhy, metaHow),
        check("search_title_match", "search", displayTitle && title && significantTokens(displayTitle).length > 0 ? "good" : "attention", "Search title matches the article", "The SEO title may differ from the H1, but it should still describe this article.", "Do not write a search title for a different topic than the body."),
    ];
}
function structureChecks(input) {
    const headings = extractHeadings(input.body);
    const h2 = headings.filter((heading) => heading.level === 2);
    const firstHeading = headings[0];
    const genericOnly = headings.length > 0 && headings.every((heading) => GENERIC_HEADING_RE.test(heading.text));
    ...
    const introStatus = !input.body.trim()
        ? "attention"
    ...
    const deliverStatus = titleTokens.length === 0 || delivered >= Math.ceil(titleTokens.length * 0.5) ? "good" : "attention";
    const closing = `${headings[headings.length - 1]?.text ?? ""} ${lastBlock(input.body)}`;
    const conclusionStatus = NEXT_STEP_RE.test(closing) || !input.body.trim() ? "good" : "attention";
    ...
    const avg = bodySentences.length === 0
        ? 0
        : bodySentences.reduce((sum, sentence) => sum + wordCount(sentence), 0) / bodySentences.length;
    const readabilityStatus = bodySentences.length === 0 || (avg <= 38 && long / Math.max(bodySentences.length, 1) <= 0.3)
        ? "good"
        : "attention";
    const grams = new Map();
    ...
    const repeated = [...grams.entries()].filter(([gram, count]) => count >= 3 && (!keyphrase || !gram.includes(keyphrase)));
    const repetitionStatus = repeated.length === 0 ? "good" : "attention";
    return [
        check("structure_headings", "structure", headingStatus, "Heading hierarchy", headingWhy, headingHow),
        check("structure_intro", "structure", introStatus, "Introduction establishes the topic", introStatus === "good"
            ? "The opening is recognizably about the same subject as the title."
            : "The opening does not mention the topic named in the title.", "In the first paragraph, say who the article is for and what they will be able to check or decide."),
        check("structure_delivers", "structure", deliverStatus, "Article delivers on its title", deliverStatus === "good"
            ? "The body uses the same specific language as the title."
            : "The title promises a topic the body barely mentions.", "Either retitle to match what you wrote, or add the missing checks the title advertised."),
        check("structure_conclusion", "structure", conclusionStatus, "Useful next steps", conclusionStatus === "good"
            ? "The ending points the reader to a next action."
            : "The article stops without telling the reader what to do with the information.", "Close with a concrete next step — request a specification, ask for a sample protocol, or open the contact form. Do not add a fake FAQ just to fill space."),
        check("structure_readability", "structure", readabilityStatus, "Readability", readabilityStatus === "good"
            ? "Sentences look scannable. This is not a grade-level ranking factor."
            : "Several sentences run very long, which is hard on a phone.", "Split long sentences. Google has no preferred word count and no required reading-grade score."),
        check("structure_repetition", "structure", repetitionStatus, "Excessive repetition", repetitionStatus === "good"
            ? "No copied 5-word run stood out."
            : `Repeated phrasing: “${repeated[0]?.[0] ?? ""}”.`, "Say it once, then move on. Do not repeat a phrase to hit a density target — Google has none."),
    ];
}
function peopleFirstChecks(input) {
    ...
    const usefulStatus = words === 0 || paragraphs.length < 2 ? "attention" : "good";
    const expertiseStatus = words === 0 ? "attention" : expertiseHits >= 2 || (expertiseHits >= 1 && hasNumbers) ? "good" : "attention";
    const originalStatus = fillerHits.length >= 2 ? "attention" : expertiseHits >= 1 || hasNumbers ? "good" : "attention";
    const beyondStatus = expertiseHits >= 2 && hasNumbers ? "good" : "attention";
    const intentStatus = extractHeadings(body).length > 0 ||
        /[?？]/.test(body) ||
        /check|verify|compare|ask|specify|بررسی|مقایسه|مشخص|спроси|сравн|провер|kiểm tra|so sánh|hỏi/i.test(body)
        ? "good"
        : "attention";
    const satisfiedStatus = NEXT_STEP_RE.test(body) || extractHeadings(body).length >= 2 ? "good" : "attention";
    const claimsStatus = /\b(iso|haccp|halal|certified|certificate number|\$|€|price guaranteed)\b/i.test(body) && !/ask|verify|request|typical|example/i.test(body)
        ? "attention"
        : "good";
    const trustStatus = trimToString(input.author) || /\/about|\/quality-control|\/certifications/i.test(body) ? "good" : "attention";
    return [
        check("people_useful", "people-first", usefulStatus, "Genuinely useful to the intended audience", usefulStatus === "good"
            ? "There is enough structured explanation for a buyer who landed here on purpose."
            : "The body is too thin to leave a reader satisfied.", "Write the checks you would actually walk through on a call. Google asks: would this audience find it useful if they came directly to you?"),
        check("people_expertise", "people-first", expertiseStatus, "Relevant / first-hand expertise", expertiseStatus === "good"
            ? "The copy uses operational language rather than generic praise."
            : "Nothing here shows that the writer has handled the product, the documents, or the cold chain.", "Add a concrete detail from export work (what you inspect, what you write into a spec, what you refuse to invent). Keep the byline as the team — do not invent a named person."),
        check("people_original", "people-first", originalStatus, "Original information or analysis", originalStatus === "good"
            ? "The article is not only filler phrasing."
            : fillerHits.length
                ? `Generic AI-style phrasing: “${fillerHits.slice(0, 2).join("”, “")}”.`
                : "The article could be a rewrite of common advice. Add something only this desk would know.", "Google asks whether the page goes beyond copying other sources. Add a worked example, a buyer checklist, or a distinction you actually use."),
        check("people_beyond", "people-first", beyondStatus, "Goes beyond rewriting common information", beyondStatus === "good"
            ? "Specific process language and figures are present."
            : "This still reads like a commodity summary.", "Name the decision the importer has to make. Do not add length for its own sake — Google has no preferred word count."),
        check("people_intent", "people-first", intentStatus, "Answers the likely reader intent", intentStatus === "good"
            ? "The structure looks like it is answering a buyer’s question."
            : "It is unclear what question a searcher would have answered.", "State the question in the title or first heading, then answer it before expanding."),
        check("people_satisfied", "people-first", satisfiedStatus, "Leaves the reader satisfied", satisfiedStatus === "good"
            ? "The reader can leave with a next action or a complete set of checks."
            : "A reader may still need to search again.", "Google’s people-first test: will someone leave feeling they learned enough to achieve their goal?"),
        check("people_claims", "people-first", claimsStatus, "Claims supported where appropriate", claimsStatus === "good"
            ? "No hard commercial claim jumped out as unsourced."
            : "Certificates, prices, or guarantees appear without “ask / verify / request”.", "Do not invent certificate numbers, prices, or customer names. Link to /certifications or tell the reader to request the current document set."),
        check("people_trust", "people-first", trustStatus, "Clear and trustworthy", trustStatus === "good"
            ? "A team byline or a link to an about / quality page is present."
            : "Readers cannot see who stands behind the article.", "Keep the organization byline. Link to /about or /quality-control. Do not invent an individual author."),
        check("people_filler", "people-first", fillerHits.length === 0 ? "good" : "attention", "Unnecessary filler / generic AI wording", fillerHits.length === 0
            ? "No stock AI phrases were detected."
            : `Filler to rewrite: “${fillerHits.slice(0, 3).join("”, “")}”.`, "Delete throat-clearing. Google evaluates whether content looks hastily produced, not which tool drafted it."),
    ];
}
function visibilityChecks(input) {
    const headings = extractHeadings(input.body);
    ...
    const concise = firstAnswer.length > 0 && wordCount(firstAnswer) <= 90 && wordCount(firstAnswer) >= 8 ? "good" : "attention";
    ...
    return [
        check("vis_crawlable", "visibility", paragraphs.length > 0 ? "good" : "attention", "Crawlable text content", paragraphs.length > 0
            ? "The article is real text in the Markdown body, which the site renders as HTML."
            : "There is almost no indexable text — images alone are not an article.", "Write the explanation as paragraphs and lists. No special AI file (llms.txt) is required; Google says not to create one."),
        check("vis_headings", "visibility", headings.some((heading) => heading.level === 2) ? "good" : "attention", "Clear headings", "Headings help people scan and help systems quote the relevant passage. This is not “chunking for AI”.", "Use Heading 2 for each real question or check. Google says there is no need to break the page into tiny AI chunks."),
        check("vis_answers", "visibility", concise, "Concise answers to important questions", concise === "good"
            ? "The first paragraph can stand on its own as an answer."
            : "Open with a direct answer, then expand. Do not rewrite the whole article “for AI”.", "Google: you do not need to write in a special way for AI Overviews or AI Mode. Write for the importer."),
        check("vis_context", "visibility", trimToString(input.category) || (input.tags ?? []).length > 0 || hasInternal ? "good" : "attention", "Useful contextual information", "Category, tags, and links to products/markets tell readers (and systems) which entity this article is about.", "Pick a category and link the product or market the article is actually about."),
        check("vis_internal", "visibility", hasInternal ? "good" : "attention", "Strong internal links", hasInternal
            ? "The article points at existing pages on this site."
            : "No internal links yet. Google asks that links be crawlable so other pages can be discovered.", "Link existing paths such as /products/frozen-chicken-feet or /quality-control. Never invent a URL."),
        check("vis_images", "visibility", trimToString(input.image) && trimToString(input.imageAlt) ? "good" : "attention", "Relevant high-quality images", "A described photograph helps people. Google recommends useful images; it does not ask for invented dimensions.", "Use a real photograph and write alt text that matches what is shown."),
        check("vis_schema", "visibility", trimToString(input.title) && trimToString(input.excerpt) && trimToString(input.image) && normalizeIsoDate(input.date)
            ? "good"
            : "attention", "Structured data matches visible content", "BlogPosting is generated automatically from the title, excerpt, featured image, dates, team byline, and URL. No extra AI schema is added.", "Keep those fields true. Do not add FAQ or other schema that is not on the page. Google says structured data is not a special AI-Overview signal."),
        check("vis_entity", "visibility", hasInternal || /\/products\/|\/markets\//.test(input.body) ? "good" : "attention", "Clear entity / topic relationships", "Linking a real product or market page is how this site expresses what the article is about.", "Add a related path or an in-body link to the product, market, or process page that already exists."),
        check("vis_unique", "visibility", wordCount(input.body) > 0 && !detectKeywordStuffing(input.body, trimToString(input.focusKeyphrase))
            ? "good"
            : "attention", "Unique useful information", "Visibility in Search and in AI features comes from non-commodity, people-first pages — not from GEO/AEO hacks.", "Add a distinction you actually use in export work. Ignore llms.txt, mention-schemes, and “AI word counts”."),
    ];
}
function analyzeKeywordPlacement(input) {
...
const INTENT_INFORMATIONAL = /how|what|when|why|vs\.?|versus|guide|checklist|compare|چگونه|چیست|چه |как |что |làm sao|cách |kiểm tra|so sánh/i;
const INTENT_TRANSACTIONAL = /buy|order|quote|rfq|price|contact|request a sample|خرید|استعلام|купить|báo giá|liên hệ/i;
const INTENT_COMMERCIAL = /spec(?:ification)?|source|supplier|inspect|grade|quality|تأمین|مشخصات|поставщик|nhà cung cấp/i;
const INTENT_NAVIGATIONAL = /feiz food|about us|contact us|درباره ما|о нас/i;
function estimateSearchIntent(input) {
    const title = `${trimToString(input.title)} ${trimToString(input.seoTitle)}`;
    const body = input.body;
    let type = "unclear";
    if (INTENT_NAVIGATIONAL.test(title))
        type = "navigational";
    else if (INTENT_INFORMATIONAL.test(title))
        type = "informational";
    else if (INTENT_TRANSACTIONAL.test(title))
        type = "transactional";
    else if (INTENT_COMMERCIAL.test(title))
        type = "commercial";
    else if (significantTokens(title).length >= 4)
        type = "informational";
    let aligned = true;
...
function keywordChecks(input) {
    ...
    const placementStatus = !keyphrase
        ? "good"
        : stuffed
            ? "attention"
            : placement.inTitle || placement.inIntro
                ? "good"
                : "attention";
    let placementWhy = "The primary keyword appears where a reader would expect the topic to be named.";
    let placementHow = "Use the phrase naturally in the title or first paragraph. Related wording is enough — do not repeat it in every heading.";
    if (!keyphrase) {
    ...
    return [
        check("kw_present", "keywords", keyphrase ? "good" : "attention", "Primary keyword is set", keyphrase
            ? "A primary keyword is set as an editorial target. It is not shown on the page."
            : "No primary keyword is set. The assistant cannot check placement until you name one.", keyphrase
        ...
        check("kw_stuffing", "keywords", stuffed ? "attention" : "good", "Keyword stuffing risk", stuffed
            ? "Exact-phrase repetition looks like it was written for search engines first."
            : "No stuffing pattern was detected. This check never requires a density percentage.", "Google’s people-first questions explicitly warn against writing to a preferred word count or repeating terms for rankings."),
        check("kw_placement", "keywords", placementStatus, "Keyword placement", placementWhy, placementHow),
    ];
}
function editorialChecks(input) {
    const words = wordCount(input.body);
    let depthStatus = "good";
    let depthWhy = "Length looks like a complete briefing for this topic. Google has no preferred word count.";
    ...
    let linkCountStatus = "good";
    let linkCountWhy = "Internal links look like a useful editorial set, not a quota.";
    let linkCountHow = "Two to five existing site pages is a useful range for this kind of article. That is an editorial suggestion, not a Google rule.";
    ...
    return [
        ...
        check("intent_align", "search", intent.aligned ? "good" : "attention", "Search intent alignment", intent.reason, "Name the job the reader came to do, then answer it before expanding. Do not write a teaser."),
    ];
}
function linkChecks(input, catalog, suggestions) {
    const links = extractMarkdownLinks(input.body);
    const internal = links.filter((link) => link.href.startsWith("/") && !link.href.startsWith("//"));
    ...
    return [
        check("link_internal", "links", unknownInternal.length > 0 ? "required" : hasInternal ? "good" : "attention", "Internal linking", unknownInternal.length > 0
            ...
            : hasInternal
                ...
                : suggestions.length
                    ...
                    : "No internal links yet. Only link pages that already exist.", "Use root-relative paths. Suggested pages come from this site’s catalog — never from invented URLs."),
        check("link_external", "links", !claimHeavy || external.length > 0 ? "good" : "attention", "External / reference links", !claimHeavy
            ? "No official-source claim jumped out. External links are optional."
            : external.length > 0
                ? "The article cites at least one https source."
                : "The article discusses official or regulatory ideas without a source the reader can open.", "When you mention a rule or document type, link a real official page — or tell the reader to request the current set. Do not invent citations."),
    ];
}
function imageChecks(input) {
    const image = trimToString(input.image);
    ...
    const altLooksStuffed = Boolean(alt) &&
        trimToString(input.focusKeyphrase) &&
        countPhrase(alt, trimToString(input.focusKeyphrase)) >= 2;
    let featuredStatus = "good";
    if (!image || !alt)
        featuredStatus = "required";
    else if (altLooksStuffed || genericName)
        featuredStatus = "attention";
    return [
        check("img_featured", "images", featuredStatus, "Featured image and meaningful alt", !image || !alt
            ...
            : altLooksStuffed
                ...
                : genericName
                    ...
                    : "Featured image and alt text are present.", "Describe the visible scene. Prefer a filename such as frozen-chicken-feet-inspection.jpg when you can control it. Do not invent pixel dimensions."),
        check("img_body", "images", missingBodyAlt.length === 0 ? "good" : "attention", "Article images have alt text", missingBodyAlt.length === 0
            ? bodyImages.length
                ? "Inline images include alt text."
                : "No inline images yet — that is fine."
            : `${missingBodyAlt.length} inline image(s) have empty alt text.`, "In the Markdown image dialog, fill the alt field with what the picture shows."),
        check("img_og", "images", ogImage && !ogAlt ? "attention" : "good", "OG / Twitter image", ogImage && !ogAlt
            ? "A custom social image is set without its own alt text."
            : ogImage
                ? "Custom social image is set. Twitter/X reuses the same fields."
                : "Social cards reuse the featured image automatically when OG fields are empty.", "If you upload a 1200×630 crop, add OG alt. Otherwise leave OG image empty — do not invent a second photograph."),
    ];
}
function collectPublishBlockers(input, options = {}) {
    return analyzeBlogQuality(input, options).blockers;
}
function categoryForCheck(item) {
    if (item.group === "keywords" || item.id.startsWith("kw_"))
        return "keywords";
...
function scoreFromChecks(items) {
    if (items.length === 0)
        return 100;
...
function statusFromChecks(items) {
    if (items.some((item) => item.status === "required"))
        return "required";
...
function buildCategoryScores(checks) {
    const ids = ["seo", "content", "keywords", "links", "structure"];
    return ids.map((id) => {
...
function editorialScore(categories, blockerCount) {
    const weight = {
        seo: 0.25,
        content: 0.25,
        keywords: 0.15,
    ...
    if (blockerCount > 0)
        return Math.min(rounded, 55);
    return Math.max(0, Math.min(100, rounded));
}
function buildMetrics(input, keyword) {
    const headings = extractHeadings(input.body);
    const links = extractMarkdownLinks(input.body);
    const seoDescription = trimToString(input.seoDescription);
    const excerpt = trimToString(input.excerpt);
...
function pickTopActions(checks) {
    const rank = (item) => {
...
function analyzeBlogQuality(input, options = {}) {
